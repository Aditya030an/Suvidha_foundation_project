...
const NavBar = () => {
  ...
  return (
    <nav className={` z-50 flex items-center justify-between px-4 h-14 lg:h-24 bg-[#20212B] ${margin ? "xl:ml-20":"ml-0" }  rounded-lg`}>
      <div className="w-1/2 sm:w-1/4">
        <Link to={"/"} onClick={()=>{handleMagin("/")}}>
          <img src={logo} className=" " />
        </Link>
      </div>
      <div className=" md:w-8/12 lg:w-6/12 h-full  hidden sm:flex ">
        <ul className=" lg:text-2xl font-medium text-white flex items-center justify-around sm:gap-3 flex-shrink w-full h-full">
          <Link to={"/gallary"} onClick={()=>{handleMagin("*")}}>
            <li className=" hover:bg-white hover:text-black px-3 py-1  xl:px-7 lg:px-5 lg:py-2 xl:py-2 rounded-full cursor-pointer transform transition-all ease-linear duration-400 delay-100">
              Gallary
            </li>
          </Link>
      ...
      <div className="z-50  w-1/4 flex items-center justify-end visible sm:hidden">
